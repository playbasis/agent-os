...
```bash
pnpm pbos operator-twin loop <mission.json> --rendered-report <path> [--visual-report <path>] [--browser-report <path>] [--leak-report <path>] [--workers N] --worker-command <cmd> [--worker-command <cmd>]
```

`operator-twin loop` runs one git worktree per worker attempt, applies only the
mission's allowed-file diff to the main worktree for scoring, then rolls losing
attempts back. Supplying one `--worker-command` reuses it for all workers;
supplying multiple commands requires one command per `--workers` slot. Candidate
scoring is driven by the mission's own `verification.commands`; supported
placeholders are `<mission.json>`, `<rendered-report.json>`,
`<visual-report.json>`, `<browser-report.json>`, and `<leak-report.json>`.
Loop reports store command hashes and stdout/stderr hashes only.
...
pnpm pbos evidence-dashboard improve [--paths 20] [--variants 6] [--critique-rounds 3] [--region-critique] [--timestamp YYYYMMDDTHHMMSSZ] [--reuse-reference <path-to-existing-reference-png>] [--skip-browser]
pnpm pbos evidence-dashboard score-app --target <path-or-url> --target-id <slug> --reuse-reference <path-to-existing-reference-png> [--repair-rounds 4] [--region-critique] [--timestamp YYYYMMDDTHHMMSSZ]
pnpm pbos knowledge compile --sources reports --out reports/agent-os-wiki
pnpm pbos knowledge lint --path reports/agent-os-wiki
```

`pbos evidence-dashboard improve --paths 20 --variants 6 --critique-rounds 3` regenerates the
evidence dashboard, browser screenshots, mission report, variant-score artifact,
improvement trace, critique lesson/memory artifacts, deterministic macro-zone
structural diagnostics, and snapshot bundle. `--reuse-reference`
points the run at an existing private PNG outside the repo and records only its
hash, byte count, and dimensions. `--region-critique` additionally asks a
vision-capable provider to localize UI defects, gated by
`PBOS_ALLOW_PROVIDER_CALLS=1`; the public artifact stores only hashes,
normalized boxes, categorical defect kinds, and hashed provider text. For slow
vision reads, raise `PB_AZURE_OPENAI_TIMEOUT_MS` in the local environment. When
live image provider configuration is available, the command asks GPT-image-2 for
a private visual reference, stores the PNG outside the repo, and records only
safe metadata such as hash, byte count, and dimensions. The harness edit
boundary is documented in
[Evidence Dashboard Harness Boundary](EVIDENCE_DASHBOARD_HARNESS.md):
for dashboard behavior, hand-edit only generator code, dashboard prompt logic,
and runner/orchestration code. CLI wiring and docs may describe or expose the
harness, but treat
`reports/evidence-improvement-report.html`, screenshots, improvement traces,
and `reports/snapshots/*` as output-only artifacts regenerated by
`pbos evidence-dashboard improve`. `--skip-browser` is a debug escape hatch; do
not use it for final dashboard promotion because it bypasses local render gates.

`pbos evidence-dashboard score-app` scores a standalone local page/app target
against a private reference image without writing the legacy dashboard HTML. It
writes target-specific screenshots, a manifest, and a score report under
`reports/app-targets/<target-id>/`; public artifacts store only target hashes,
reference hashes/dimensions, aggregate visual scores, structural diagnostics,
browser-gate results, and optional hash-only region critique metadata.
`--repair-rounds` scores source-controlled app variants, accepts only
non-regressing candidates, reverts lower-scoring candidates in the accepted
series, and writes `repair-loop-proof-<timestamp>.json`,
`repair-loop-lesson-<timestamp>.json`, and `repair-loop-memory.json`.
